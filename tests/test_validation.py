...
def test_aggregate_quantity_check(patch_db):
    ...
    _, name_map = _resolve_item_names(line_items)
    issues, stock_checks = _check_aggregate_stock(line_items, name_map)
...
def test_duplicate_detection(patch_db):
    record_invoice("INV-1004", "Precision Parts Ltd.", 1890.0, "approved", db_path=patch_db)
    issues = _check_duplicate("INV-1004")
    assert len(issues) == 1
    assert "INV-1004" in issues[0]


def test_no_duplicate_for_fresh_invoice(patch_db):
    assert _check_duplicate("INV-FRESH") == []


def test_negative_quantity():
    issues = _check_negative_quantities(
        [{"item_name": "WidgetA", "quantity": -5.0, "unit_price": 250.0}]
    )
    assert len(issues) == 1
...
def test_zero_quantity_also_flagged():
    issues = _check_negative_quantities(
        [{"item_name": "WidgetB", "quantity": 0.0, "unit_price": 500.0}]
    )
    assert len(issues) == 1


def test_unknown_item(patch_db):
    issues, name_map = _resolve_item_names(
        [{"item_name": "SuperGizmo", "quantity": 1.0, "unit_price": 999.0}]
    )
...
def test_price_variance(patch_db):
    ...
    _, name_map = _resolve_item_names(line_items)
    issues = _check_price_variance(line_items, name_map)
    assert len(issues) == 1
    assert "WidgetA" in issues[0]


def test_price_variance_within_tolerance(patch_db):
    line_items = [{"item_name": "WidgetA", "quantity": 5.0, "unit_price": 255.0}]
    _, name_map = _resolve_item_names(line_items)
    assert _check_price_variance(line_items, name_map) == []
